...
for (i in seq_along(oos_idx)) {
  ...
  train_clean <- train[complete.cases(train[, c("ise_ldiff", lag_cols)]), ]
  ...
  formula <- as.formula(paste("ise_ldiff ~", paste(lag_cols, collapse = " + ")))
...
# ── Compile results ────────────────────────────────────────────────────────────
results <- df[oos_idx, c("date", "ise_ldiff")] |>
  mutate(
    forecast_ar = forecasts,
    model       = "AR"
  ) |>
  rename(actual = ise_ldiff)
