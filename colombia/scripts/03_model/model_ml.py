...
for k in range(1, 5):
    df[f"ise_lag{k}"] = df["ise_ldiff"].shift(k)
    all_features.append(f"ise_lag{k}")
...
for i, t in enumerate(oos_idx):
    ...
    trad_complete = train[trad_vars + ["ise_ldiff"]].notna().all(axis=1)
    ...
    y_train = train["ise_ldiff"].values
...
# ── Compile results ───────────────────────────────────────────────────────────
out = df.iloc[oos_idx][["date", "ise_ldiff"]].copy().reset_index(drop=True)
out = out.rename(columns={"ise_ldiff": "actual"})
