...
fig_dir <- here("colombia/outputs", "figures")
dir.create(fig_dir, showWarnings=FALSE, recursive=TRUE)

# ── Load data ─────────────────────────────────────────────────────────────────
ar    <- read_csv(here("colombia/data/final/forecasts_ar.csv"), show_col_types=FALSE)
dfm_b <- read_csv(here("colombia/data/final/forecasts_dfm_benchmark.csv"), show_col_types=FALSE)
dfm_a <- read_csv(here("colombia/data/final/forecasts_dfm_augmented.csv"), show_col_types=FALSE)
ml    <- read_csv(here("colombia/data/final/forecasts_ml.csv"), show_col_types=FALSE)
errors <- read_csv(here("colombia/outputs/tables/forecast_errors.csv"), show_col_types=FALSE)
...
p1 <- ggplot(fc_long, aes(x=date, y=value, color=model, linetype=model)) +
  ...
  labs(
    ...
    subtitle = "Monthly log-growth of ISE, pseudo out-of-sample 2018–2024",
    x        = NULL,
